...
let MozLoopServiceInternal = {
  ...
  get expiryTimeSeconds() {
    ...
    try {
      expiryTimeSeconds = Services.prefs.getIntPref("loop.urlsExpiryTime");
    } catch (x) {
      // It is ok for the pref not to exist.
    }
  ...
  set expiryTimeSeconds(time) {
    if (time > this.expiryTimeSeconds) {
      Services.prefs.setIntPref("loop.urlsExpiryTime", time);
    }
  },
  ...
  onPushRegistered: function(err, pushUrl) {
    ...
    // work.
    this.loopXhr.channel.QueryInterface(Ci.nsIHttpChannelInternal);
    this.loopXhr.channel.forceAllowThirdPartyCookie = true;
